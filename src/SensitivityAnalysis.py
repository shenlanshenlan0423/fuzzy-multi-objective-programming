...
if __name__ == '__main__':
    ...
    beta_table = np.hstack((np.array([0.1, 0.2, 0.6]).reshape(-1, 1), np.array(beta_ms_arr)))
    beta_table = pd.DataFrame(beta_table, columns=['Beta_ms', 'k1', 'k2', 'k3', 'k4', 'k5', 'S', 'U', 'C', 'V'])
    beta_table.to_excel(TABLE_DIR + '/beta_ms_table.xlsx', index=False)
    print(beta_table.to_markdown())
    ...
    beta_table = np.hstack((np.array([0.3, 0.5, 0.8]).reshape(-1, 1), np.array(beta_v1_arr)))
    beta_table = pd.DataFrame(beta_table, columns=['Beta_v1', 'k1-V', 'k2-V', 'k3-V', 'k4-V', 'k5-V'])
    beta_table.to_excel(TABLE_DIR + '/beta_v1_table.xlsx', index=False)
    print(beta_table.to_markdown())
